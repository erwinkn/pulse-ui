...
- Inputs the user edits before hydration are restored by the pre-hydration capture script (emitted in the SSR document / `PulseProvider`, replayed after the hydration commit). Caveat: inputs inside lazy/suspended subtrees that hydrate after the root commit can still lose pre-hydration edits.
