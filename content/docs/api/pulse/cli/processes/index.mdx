...
<PyAttribute name={"ANSI_CODES"} type={null} value={"{'cyan': '\\x1b[36m', 'orange1': '\\x1b[38;5;208m', 'reset': '\\x1b[0m'}"} />

<PyAttribute name={"TAG_COLORS"} type={null} value={"{'server': 'cyan', 'web': 'orange1'}"} />

<PyAttribute name={"ANSI_ESCAPE"} type={null} value={"re.compile('\\\\x1B(?:[@-Z\\\\\\\\-_]|\\\\[[0-?]*[ -/]*[@-~])')"} />

<Tabs items={["Functions"]}>

<Tab value={"Functions"}>

<PyFunction name={"execute_commands"} type={"(commands, *, tag_mode='colored') -> int"}>
...
<PyParameter name={"commands"} type={"Sequence[CommandSpec]"} value={undefined}>

List of command specifications to run

</PyParameter>
<PyParameter name={"tag_mode"} type={"TagMode"} value={"'colored'"}>

How to display process tags:
- "colored": Show [server]/[web] with ANSI colors (dev mode)
- "plain": Show [server]/[web] without colors (ci/prod mode)
...
<PyFunction name={"_call_on_spawn"} type={"(spec) -> None"}>
...
<PyParameter name={"spec"} type={"CommandSpec"} value={null} />
...
<PyFunction name={"_check_on_ready"} type={"(spec, line, ready_flags, key) -> None"}>
...
<PyParameter name={"spec"} type={"CommandSpec"} value={null} />
<PyParameter name={"line"} type={"str"} value={null} />
<PyParameter name={"ready_flags"} type={"dict[_K, bool]"} value={null} />
<PyParameter name={"key"} type={"_K"} value={null} />
...
<PyFunction name={"_run_with_pty"} type={"(commands, *, tag_mode) -> int"}>
...
<PyParameter name={"commands"} type={"Sequence[CommandSpec]"} value={null} />
<PyParameter name={"tag_mode"} type={"TagMode"} value={null} />
...
<PyFunction name={"_run_without_pty"} type={"(commands, *, tag_mode) -> int"}>
...
<PyFunction name={"_write_tagged_line"} type={"(name, message, tag_mode) -> None"}>
...
<PyParameter name={"tag_mode"} type={"TagMode"} value={undefined}>

How to display the tag:
- "colored": Show [name] with ANSI colors
- "plain": Show [name] without colors
...
<PyFunction name={"_close_fd"} type={"(fd) -> None"}>
...
<PyParameter name={"fd"} type={"int"} value={null} />
