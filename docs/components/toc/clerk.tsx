	...
	useEffect(() => {
		...
		function onResize(): void {
			...
			for (let i = 0; i < items.length; i++) {
				...
				d.push(`${i === 0 ? "M" : "L"}${offset} ${bounds.top}`);
				d.push(`L${offset} ${bounds.bottom}`);
			}
				...
				<div
					...
					style={{
						...
						maskImage: `url("data:image/svg+xml,${
							// Inline SVG
							encodeURIComponent(
								`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${svg.width} ${svg.height}"><path d="${svg.path}" stroke="black" stroke-width="${ACTIVE_STROKE_WIDTH}" fill="none" /></svg>`,
							)
						}")`,
					}}
				>
