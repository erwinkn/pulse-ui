...
Several live handles can share one name. Events fan out to all attached handles. A request goes to just one: the first attached handle that has a handler for that event, in registration order.
