...
Create a channel inside a component using `ps.channel()`. The default lifetime is
the current route.
...
The string argument (`"chat"`) is an identifier that helps with debugging. If you
do not provide one, Pulse generates a UUID. Pulse closes the channel when it
destroys the route.
...
On the client, select the same lifetime so the subscription does not carry a
route owner:

```python
bridge = usePulseChannel(channelId, "tab")
```

A tab channel belongs to one render session, which normally means one browser
tab. It is not shared with other tabs or users. Pulse closes it when the render
session ends. Its handlers do not have a current route because the channel has no
route owner.
...
```python
from pulse import ChannelDisconnected, ChannelTimeout

async def safe_request(chat: ps.Channel):
    try:
        response = await chat.request("get_data", timeout=5.0)
        return response
    except (ChannelDisconnected, ChannelTimeout):
        # Client didn't respond in time
        return None
```

Catch `ChannelDisconnected` when the channel still exists but no browser bridge is
subscribed. Catch `ChannelClosed` when the route or render session has destroyed the
channel, or after calling `channel.close()`.
