...
| `on_dispose()` | When component unmounts | Clean up resources, cancel subscriptions |
...
Every hook state owns a `ResourceScope`. Resources you create inside the factory (effects, states) are captured into it automatically, and it is disposed when the component unmounts—you don't need to track them yourself:

```python
class PollerHookState(ps.hooks.State):
    def __init__(self):
        # Owned by the hook state, disposed on unmount
        self.poller = ps.Effect(self._poll, interval=5.0)
```

For resources created later (outside the factory), attach them explicitly:

```python
def start(self):
    self.task = ps.Effect(self._run, lazy=True)
    self.resources.own(self.task)
```

`dispose()` itself cannot be overridden—it runs your `on_dispose()`, then disposes the owned resources. This guarantees cleanup even if `on_dispose()` raises.
