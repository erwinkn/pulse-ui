...
Decision hooks must return `ps.Ok(...)` or `ps.Deny()`. Returning anything else raises
`ps.MiddlewareDecisionError` and fails closed; for `connect`, the connection is refused.
Other connect middleware exceptions are reported to the client after it binds.

### `message`

Called for each client-initiated WebSocket command — clicks, form submissions, navigation, attach, and so on. `{type: "reply"}` completions skip this hook and resolve immediately. They're the other half of a server request (`call_api`, `eval_js`, `Channel.request`), not something you authorize.
