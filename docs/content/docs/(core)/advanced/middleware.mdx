...
Called for pub/sub channel messages — both when a client subscribes and when it sends data. Useful for authorizing access to real-time channels:

```python
async def channel(self, *, channel_id, event, payload, session, next):
    # Lifecycle: event is "connect" or "disconnect", payload is the owner
    if event == "connect":
        if channel_id.startswith("admin:") and not session.get("is_admin"):
            return ps.Deny()  # subscription rejected
        return await next()

    # Data-plane: event is the channel event name ("message", "syncValues", …)
...
`event` is `"connect"` or `"disconnect"` for lifecycle, or the data event name otherwise. `ps.Deny()` on connect rejects the subscription. `ps.Deny()` on disconnect still disconnects — you can't keep a client subscribed.
