...
Called for each client-initiated WebSocket command — clicks, form submissions, navigation, attach, and so on. `{type: "reply"}` completions skip this hook and resolve immediately. They're the other half of a server request (`call_api`, `run_js`, `Channel.request`), not something you authorize.
