...
You can also create refs outside render. They start inert and only attach when you pass them to an element. Each call returns a new handle (no hook identity), which you can pass into a component later. When the element unmounts, the ref detaches automatically.
