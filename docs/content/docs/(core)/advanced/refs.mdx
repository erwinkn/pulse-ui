...
Under the hood, the `ref` prop is serialized as an eval-only token: `#ref:{channelId},{refId}`. You don't need to construct this yourself.
...
Refs are inert at creation, both inside and outside render. Render only provides stable identity (callsite or `key`). A ref becomes active only when you attach it via `ref=` or a ref callback.

You can create refs outside render too. Each call returns a new handle (no hook identity), which you can pass into a component later. When the element unmounts, the ref becomes unmounted but stays attached; it detaches only when it disappears from the latest render or the route/session closes.
