...
Pulse generates route manifests and layout files that connect your Python routes to the web frontend. This happens automatically during development and can be triggered manually for production builds.

### How it works

When you run `pulse dev app.py`, Pulse:

1. Reads your route definitions from the `App` object
2. Generates route manifests + layout files in your web directory
...
5. Starts the Python server, Vite dev server, and SSR server
...
For production builds or CI, call `run_codegen()` directly (or use `pulse build`):
...
Your Pulse server handles everything: the API, WebSocket connections, and serving the web frontend.
...
Build the frontend, then start the server + SSR process:

```bash
pulse build app.py
pulse start app.py --address 0.0.0.0 --port 8000
```
