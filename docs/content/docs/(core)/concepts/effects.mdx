...
Effects created inside a `ps.init()` block or `ps.setup()` initializer belong to that initializer. Their cleanup runs when its key changes, when initialization fails, or when the component unmounts.
