...
If you want sessions to live on the server, implement `ps.SessionStore` with your own backend:
...
class RedisSessionStore(ps.SessionStore):
    ...
    async def get(self, sid: str) -> dict[str, object] | None:
        payload = await self.redis.get(f"session:{sid}")
        return None if payload is None else json.loads(payload)

    async def create(self, sid: str) -> dict[str, object]:
        session: dict[str, object] = {}
        await self.save(sid, session)
        return session

    async def save(self, sid: str, session: dict[str, object]) -> None:
...
<Callout type="warn">
If you opt into server-backed sessions, make sure every app instance can reach the same backend. Otherwise users can lose their sessions when traffic moves between processes or deployments.
</Callout>
