...
If you need direct access to the shared app store, use `ps.store()`.
...
By default, Pulse stores sessions in signed cookies. That's true in development and production unless you opt into a server-backed `ps.SessionStore`.

Pulse also has an app-level store primitive. In development, `ps.App()` creates a local SQLite-backed app store by default, so `ps.store()` works without extra setup.

If you want the same store for sessions and other app state, pass a `ps.SessionStore()` and let it inherit the app store:
...
If you want sessions in a different backend, give the session store its own KV store:
...
app = ps.App(
    routes=[...],
    store=ps.SQLiteKVStore(".pulse/app.sqlite3"),
    session_store=ps.SessionStore(
        store=ps.RedisKVStore.from_url("redis://localhost:6379/1"),
    ),
)
```

`ps.SessionStore(store=...)` uses its own KV store first, then falls back to `ps.App(store=...)` when no store is provided.

<Callout type="warn">
If you opt into server-backed sessions in development, multiple Pulse apps in the same working directory can share the same default local store. That's convenient for reloads, but it can also make dev sessions bleed across apps that reuse the same host and cookie name.
</Callout>
...
This is more maintainable than adding auth checks to every protected component. See the [Middleware guide](/docs/advanced/middleware) for more details.
