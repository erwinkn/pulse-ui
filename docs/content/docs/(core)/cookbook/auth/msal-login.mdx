...
The login link stays relative to the current origin. In production, set `PULSE_PUBLIC_ORIGIN` so MSAL can construct the absolute callback URI. Use `App(public_origin=...)` only for a programmatic override. The plugin only accepts same-origin relative `next` paths.
