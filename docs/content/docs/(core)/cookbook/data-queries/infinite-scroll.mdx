...
Show placeholder content immediately while the first page loads:
...
def _initial_feed(self):
    return [Page(data={"items": [], "next_cursor": 0, "prev_cursor": None}, param=0)]
```

## Avoiding flicker on key changes

Use `keep_previous_data=True` to keep showing old data while a new feed loads (e.g., when switching feed types):

```python
@ps.infinite_query(initial_page_param=0, keep_previous_data=True)
async def feed(self, page_param: int) -> FeedPage:
    ...
...
When `feed_type` changes, the old feed remains visible until the new one loads.
