...
Use `eval_js` to await a value from JavaScript:
...
class TrackerState(ps.State):
    ...
    async def check_scroll(self):
        pos = await ps.eval_js(get_scroll_position())
        self.scroll_x = pos["x"]
        self.scroll_y = pos["y"]

    async def capture_selection(self):
        self.selection = await ps.eval_js(get_selected_text())
```
...
class ShareState(ps.State):
    ...
    async def copy_link(self, url: str):
        await ps.eval_js(copy_to_clipboard(url))
        self.copied = True
```
...
class PreferencesState(ps.State):
    ...
    async def load_theme(self):
        stored = await ps.eval_js(load_from_storage("theme"))
        if stored:
            self.theme = stored
...
async def safe_js_call():
    try:
        result = await ps.eval_js(some_js_function())
