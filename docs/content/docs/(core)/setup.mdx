...
Register Pulse's Vite plugin in `web/vite.config.ts`. The development
supervisor uses it to learn when Vite is actually listening:
...
The `pulse run` command starts the Python and React Router servers. Pulse keeps the public port reserved while you work, starting with 8000 and choosing the first available port. When you save Python, Pulse restarts both servers and regenerates the routes without handing that port between processes. Vite stays on loopback behind the proxy in single-server mode, then reloads the browser when the new stack is ready. In-memory Pulse state resets. JavaScript, TypeScript, and CSS changes still use Vite's normal HMR.
