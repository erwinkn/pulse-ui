...
Add the Pulse development bridge in `web/vite.config.ts`:
...
Pulse uses this bridge through Vite's existing development server for Python reloads. Vite keeps handling ordinary frontend HMR. If you forget this step, don't worry—`pulse run` checks for the plugin at startup and adds it to your Vite config automatically.
...
The `pulse run` command starts the Python and React Router servers. When you save Python, Pulse prepares the new backend and routes together, then reloads the browser after both are ready.
