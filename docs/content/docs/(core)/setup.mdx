...
Register Pulse's Vite plugin in `web/vite.config.ts`. Pulse waits for it
to report that Vite is actually listening before serving traffic — hitting
`:8000` too early is what used to throw those confusing first-load errors.
The plugin also points HMR at the public port in single-server mode.
...
The `pulse run` command starts the Python and React Router servers. Pulse holds the public port (starting at 8000, first one free) and restarts only the Python worker when you save. Vite stays up. In single-server mode it listens on loopback behind the proxy. WebSockets and in-memory Pulse state reset on a Python save; JavaScript, TypeScript, and CSS still use Vite HMR.
