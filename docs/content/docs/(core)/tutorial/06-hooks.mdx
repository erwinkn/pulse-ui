...
`ps.init()` and `ps.state()` accept an optional `key` argument. When the key changes, the hook re-initializes:

```python
@ps.component
def DataLoader(data_id: int):
    # Re-runs load_data whenever data_id changes.
    with ps.init(key=str(data_id)):
...
Keys must be non-empty strings, so build stable string keys from the values that identify the instance.
