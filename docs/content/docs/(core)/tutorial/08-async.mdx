...
5. **Use Pulse's scheduling for timers and tasks**: Instead of `asyncio.create_task()`, use `ps.later()` and `ps.repeat()` for timers. These are automatically cleaned up when the render session closes. Route unmounting does not cancel `ps.later()` callbacks, so cancel the returned handle when a route-local callback should not run after unmount.

## See also

- [Scheduling reference](/docs/reference/pulse/scheduling) - timers, repeating callbacks, and tracked tasks
- [Core Concepts: Async](/docs/concepts/async)
