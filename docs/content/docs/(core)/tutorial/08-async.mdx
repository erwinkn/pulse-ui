...
5. **Use Pulse's scheduling for timers and tasks**: Instead of `asyncio.create_task()`, use `ps.later()` and `ps.repeat()` for timers. These are automatically cleaned up when the render session closes—no need for manual cancellation in most cases.
