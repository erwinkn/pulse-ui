...
`ps.navigate()` is route-bound by default. If the user leaves the route before an
async handler finishes, the navigation is ignored. Use
`ps.navigate("/dashboard", force=True)` only when the redirect should still win
after the route has unmounted.
...
def user_profile():
    ...
    route = ps.route()
    user_id = route["pathParams"].get("id")
...
def current_page():
    ...
    return ps.div(
        ...
        ps.p(f"Route params: {route['pathParams']}"),
    )
```
