...
For operations that return values, use `eval_js` and await:

```python
from pulse.js import window

async def get_window_size(self):
    width = await ps.eval_js(window.innerWidth)
    height = await ps.eval_js(window.innerHeight)
...
class EditorState(ps.State):
    async def copy_selection(self):
        text = await ps.eval_js(get_selection())
        if text:
