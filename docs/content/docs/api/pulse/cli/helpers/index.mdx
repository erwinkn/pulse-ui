...
<Card title={"ParsedAppTarget"} href={"/docs/api/pulse/cli/helpers/ParsedAppTarget"} />
...
<PyFunction name={"os_family"} type={"() -> Literal['windows', 'mac', 'linux']"}>
...
<PyFunctionReturn type={"typing.Literal['windows', 'mac', 'linux']"} />

</PyFunction>
<PyFunction name={"_module_path_from_file"} type={"(file_path) -> tuple[str, Path]"}>
...
<PyParameter name={"file_path"} type={"Path"} value={null} />

</div>

<PyFunctionReturn type={"tuple[str, pathlib.Path]"} />

</PyFunction>
<PyFunction name={"parse_app_target"} type={"(target) -> ParsedAppTarget"}>
...
<PyFunctionReturn type={"pulse.cli.helpers.ParsedAppTarget"} />

</PyFunction>
<PyFunction name={"load_app_from_target"} type={"(target, logger=None) -> AppLoadResult"}>
...
The app target string (file path or module path)

</PyParameter>
<PyParameter name={"logger"} type={"CLILogger | None"} value={"None"}>

Optional CLILogger for error output. If not provided, uses basic print/traceback.

</PyParameter>

</div>

<PyFunctionReturn type={"pulse.cli.models.AppLoadResult"} />

</PyFunction>
<PyFunction name={"ensure_gitignore_has"} type={"(root, *patterns) -> None"}>
...
<PyParameter name={"root"} type={"Path"} value={undefined}>

Directory containing (or to contain) .gitignore

</PyParameter>
<PyParameter name={"patterns"} type={"str"} value={"()"} />
...
<PyFunction name={"install_hints_for_mkcert"} type={"() -> list[str]"}>
