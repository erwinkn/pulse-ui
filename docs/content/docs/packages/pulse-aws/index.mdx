...
- `WEB_ROOT`: relative path to your React Router app.

Generated files are origin-neutral. At runtime, `pulse-aws deploy` sets `PULSE_PUBLIC_ORIGIN=https://<domain>` on the ECS task; the combined Pulse process configures its internal React Router upstream itself.
