...
- AWS CDK CLI on `PATH`, or a wrapper you will pass via `--cdk-bin`
- `uv` (to install/run `pulse-aws`)
- A domain you control (for HTTPS)

## What you are deploying

Pulse deploys as a Python server plus a Bun SSR server. They can be bundled into one container (this tutorial) or split later.
...
Target Group -> ECS Service -> Fargate Tasks
  |
  v
Pulse app (Python server + Bun SSR server)
...
{/* TODO: verify this Dockerfile against the new web project layout (index.html,
    src/, server/ssr.ts, vite.config.ts, dist/client + dist/server build output)
    by running a full pulse-aws deploy. Updated from the React Router version
...
COPY --from=codegen /app/${WEB_ROOT}/index.html ./
...
COPY --from=js-build /app/${WEB_ROOT}/public /app/${WEB_ROOT}/public
...
- `WEB_ROOT`: relative path to your web project.
...
Advanced overrides:
- `--cdk-bin /path/to/wrapper` lets you use a wrapper or alternate CDK executable.
- `--cdk-workdir /path/to/cdk-app` tells `pulse-aws` to use a custom CDK app instead of the packaged one under `pulse_aws/cdk`.
- Host paths like `--dockerfile`, `--context`, and `--cdk-workdir` resolve from the directory where you run `pulse-aws`.
...
You should now see checks for the base URL, health endpoint, and (if multiple versions exist) query-based affinity routing for existing sessions via `/_pulse/meta`.

`affinity` is enforced with the `pulse_deployment` query param from existing session directives, while new tabs/sessions without that query use the default listener action (latest deployment).
