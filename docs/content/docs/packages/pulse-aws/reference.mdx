...
- adds `X-Pulse-Render-Affinity` to prerender headers and Socket.IO directives for existing render sessions. New tabs/sessions do not send this header unless they reuse those same directives.
- Uses ALB target-group `lb_cookie` stickiness to keep requests on the same ECS task within a matched deployment.
- New sessions without the affinity header follow the default listener action, which is the latest deployment.
