...
| `--domain` | `PULSE_AWS_DOMAIN` | none | required; used for ACM and `PULSE_PUBLIC_ORIGIN` |
...
| `--task-env` | none | none | repeatable `KEY=VALUE`; `PULSE_PUBLIC_ORIGIN` is managed by `--domain` |
...
- Docker build args always include `APP_FILE` and `WEB_ROOT`.
- ECS task env always includes `PULSE_PUBLIC_ORIGIN=https://<domain>`.
