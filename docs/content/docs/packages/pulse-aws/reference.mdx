...
- adds `X-Pulse-Render-Affinity` to prerender directives, sets a `pulse_affinity` cookie for browser routing, and relies on ALB target stickiness to keep requests on the same ECS task
