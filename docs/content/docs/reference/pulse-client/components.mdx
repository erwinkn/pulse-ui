...
Pulse always connects to the current origin. Framework HTTP endpoints use the fixed `/_pulse` prefix and Socket.IO uses `/_pulse/socket.io`.
...
4. If not `event.defaultPrevented`, sends form data to the same-origin action
