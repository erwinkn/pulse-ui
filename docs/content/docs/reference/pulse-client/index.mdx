...
React App                          Pulse Server
    |                                   |
    |-- attach (viewId, revision) ------>|
    |<------ attach_ack / snapshot -----|
    |                                   |
    |<-- vdom_update (base -> revision)-|  (on state change)
    |                                   |
    |-- callback (viewId, id, args) --->|  (on user interaction)
    |                                   |
```
...
Each mounted view has a stable ID and monotonic revision. If a reconnect misses an update, the revision gap is detected and the server sends an authoritative snapshot before callbacks resume.
