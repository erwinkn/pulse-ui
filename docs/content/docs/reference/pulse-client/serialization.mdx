...
Builds a `RouteInfo` object from a location plus path params. Use this when you need to construct route context manually (for example, in a custom router or SSR pipeline).
...
const routeInfo = buildRouteInfo(
  { pathname: "/users/123", search: "?tab=settings", hash: "" },
  { id: "123" },
  [],
);
```
