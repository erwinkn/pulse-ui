...
During serialization, a global counter increments for each visited payload node, including primitives. The metadata arrays record which indices are special types:
...
// Index 3: number 1 inside the Set
// Index 4: number 2 inside the Set
...
```ts
import { FormSubmissionError, submitForm } from "pulse-ui-client";

async function submitForm(options: SubmitFormOptions): Promise<void>
```

Rejects with `FormSubmissionError` when the form endpoint returns a non-2xx response.
...
type SubmitFormOptions = SubmitFormBase & (
...
| `values` | `unknown` | Optional structured values encoded by Pulse |
| `force` | `boolean` | Submit even if `defaultPrevented` |

Pass either `formData` or `values`, never both. When neither is provided, Pulse reads native controls from the submitted form.
