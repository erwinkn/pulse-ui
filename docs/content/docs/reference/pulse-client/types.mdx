...
Per-render connection directives. `query` is carried by client prerenders and
same-origin `PulseForm` submissions. Socket.IO uses its nested authentication and
query values.
