...
Props for the exported `ServerErrorOverlay` component.
...
Queue entry for the dev error overlay.

```ts
interface ServerErrorOverlayEntry {
  error: ServerError;
  fingerprint: string;
  repeatCount: number;
  receivedAt: number;
...
State shape used by `reduceServerErrorOverlay`.

```ts
interface ServerErrorOverlayState {
