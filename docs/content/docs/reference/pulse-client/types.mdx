...
Fire-and-forget channel event sent to the server.
...
Channel request awaiting a server response.
...
```ts
type ClientChannelResponseMessage = {
...
Result of JavaScript execution requested by the server.

```ts
type ClientJsResultMessage = {
  type: "js_result";
  id: string;
} & (
  | { ok: true; result: any }
...
interface ServerError {
  message: string;
  stack: string;
  phase: "render" | "callback" | "mount" | "unmount" | "navigate" | "server" | "effect" | "connect";
...
Fire-and-forget channel event from the server.
...
Channel request awaiting a client response.
...
```ts
type ServerChannelResponseMessage = {
  type: "channel_response";
  channel: string;
  responseTo: string;
} & (
...
Optional client reconstitution for `$v` (snapshot-rendered Pulse UI) markers.
...
type WireValue = Primitive | WireValue[] | { [k: string]: WireValue };
```
