...
- Subscript: `obj[key]`, `obj[start:end]` (negative indices pass through)
...
Negative indices are not rewritten. Use `.at(-1)` or `arr[arr.length - 1]` if you want array-last behavior.
