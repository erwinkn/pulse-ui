...
| `public_origin` | `str` | `None` | Optional canonical browser origin for absolute server-side URLs; `PULSE_PUBLIC_ORIGIN` is the fallback |
| `not_found` | `str` | `"/not-found"` | Path for 404 page |
| `proxy` | `WebProxyConfig` | `None` | Optional internal React Router proxy tuning |
| `socketio_options` | `dict[str, Any]` | `None` | Extra options for the Socket.IO server, e.g. `cors_allowed_origins` when a fronting proxy cannot forward `Host`/`X-Forwarded-Proto` |
...
| `public_origin` | `str \| None` | Optional canonical browser-visible origin |
...
Initialize framework routes, middleware, sessions, Socket.IO, and the optional web proxy. Called automatically by `asgi_factory`.
...
Configuration for the optional internal React Router proxy.
...
Pass it to `App(proxy=ps.WebProxyConfig(...))`. Pulse constructs `WebProxy` when `PULSE_WEB_UPSTREAM` is set. If no upstream is configured, Pulse serves only framework and FastAPI endpoints.

`WebProxy` preserves the browser-visible host while forwarding HTTP and development WebSocket traffic. Redirects that point at the internal upstream are rewritten to origin-relative locations.
