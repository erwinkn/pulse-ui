...
FastAPI's generated API reference also uses this namespace by default:

- Swagger UI: `/_pulse/docs`
- OpenAPI schema: `/_pulse/openapi.json`

ReDoc is disabled by default. Enable it or configure different URLs with
`FastAPIConfig`:
...
The generated schema includes endpoints registered on `app.fastapi` and omits
Pulse's framework endpoints. Custom URLs outside `/_pulse/*` can shadow Pulse
page routes at the same path.
...
class FastAPIConfig:
    ...
    description: str = ""
    version: str = "0.1.0"
    openapi_url: str | None = "/_pulse/openapi.json"
    docs_url: str | None = "/_pulse/docs"
    redoc_url: str | None = None
    swagger_ui_oauth2_redirect_url: str | None = "/_pulse/docs/oauth2-redirect"
...
Set `docs_url` and `openapi_url` to `None` to disable the generated reference.
Set `redoc_url` to a path to enable ReDoc. Pulse owns FastAPI routing order and
lifecycle; configure advanced runtime behavior directly through `app.fastapi`.
