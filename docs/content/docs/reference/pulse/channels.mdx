...
Calls without a payload pass `None` to the handler.
...
**Parameters:**
- `event` - Event name.
- `payload` - Data to send. Defaults to `None`, which sends JSON `null`.
...
Handlers that do not return a value respond with JSON `null`, which arrives as
