...
In dev mode, `pulse run` watches Python files and restarts the development stack on each save. It stops Vite and Uvicorn, then starts a fresh backend worker to regenerate the routes. The supervisor keeps the selected public port for its entire lifetime and forwards it to each new Uvicorn worker, avoiding port handoff races. With `--find-port`, it binds the first available port starting at `--port` (8000 by default).
