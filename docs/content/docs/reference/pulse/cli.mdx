...
In dev mode, `pulse run` watches Python files and coordinates each reload. It prepares the next backend and generated routes in isolation, confirms the Vite bridge is ready, stops the previous backend with a finite drain deadline, starts the replacement, then publishes the generated files and asks Vite for one full browser reload. Reload failures never tear down the session: failed imports or code generation keep the last good generation running, a crashed backend restarts on your next save, and if Vite briefly misses a commit the backend keeps serving—refresh the browser manually if it looks stale.

Vite still owns ordinary JavaScript, TypeScript, and CSS HMR. Add the Pulse plugin in `vite.config.ts` so generated routes only update through the coordinated commit (`pulse run` adds it automatically if it's missing):
