...
In the default single-server mode, Vite listens on loopback behind Uvicorn's proxy; port 5173 is not exposed publicly. Add `pulseVitePlugin()` from `pulse-ui-client/vite` so the browser's HMR client reconnects on the public port. Split/subdomain apps bind Vite to `--address` instead.
