...
In dev mode, `pulse run` watches Python files and restarts only the Uvicorn worker on each save. The supervisor holds the public listen sockets for the lifetime of `pulse run` and hands those inherited fds to each new worker, so the port never goes idle. Vite starts once. The Python watcher ignores `web/` — that's Vite's tree, including generated routes under `web/app/pulse/`. Files Pulse imported from Python (registered local assets) still retrigger a worker restart so codegen can recopy them. Everything else in `web/` is Vite HMR. The previous worker is SIGKILL'd immediately — WebSockets die anyway, and graceful drain just delayed the next boot.

With `--find-port` (the default), Pulse binds the first available public port starting at `--port` (8000 by default). Vite uses the first free port from 5173 with `--strictPort`.

In the default single-server mode, Vite listens on loopback behind Uvicorn's proxy; port 5173 is not exposed publicly. `pulse()` from `pulse-ui-client/vite` tells Pulse when Vite is actually listening (not just when it printed a URL) and points the HMR client at the public port. Pulse keeps that ready pipe open for Vite's lifetime so an in-process restart (editing `vite.config.ts`) does not break the handshake. Split/subdomain apps bind Vite to `--address` instead. Pulse still generates routes once before Vite starts so `web/app/routes.ts` can import `./pulse/routes` on a fresh checkout. The worker regenerates on each Python restart.

If Python import, codegen, or startup fails, Vite keeps running and Pulse waits for the next save. If Vite itself exits, `pulse run` exits with Vite's code. JavaScript, TypeScript, and CSS changes still use Vite's normal HMR without restarting Python.
