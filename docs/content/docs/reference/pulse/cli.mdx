...
In the default single-server mode, Vite listens on loopback behind Uvicorn's proxy; port 5173 is not exposed publicly. `pulseVitePlugin()` from `pulse-ui-client/vite` tells Pulse when Vite is actually listening (not just when it printed a URL) and points the HMR client at the public port. Split/subdomain apps bind Vite to `--address` instead.
