...
pulse/
  ...
  routes.ts          # Pulse route manifest + loader map
  routes/
    index.jsx        # Index route
    users.jsx        # /users route
    users/_id_<hash>.jsx  # /users/:id route (sanitized)
  layouts/
    layout/_layout.tsx    # Layout components
  assets/
...
Generate the Pulse route manifest file.
