...
- Same-origin `/_pulse` requests
- Runtime `PULSE_SSR_BACKEND_URL` lookup for server-side prerendering
...
```python
def generate_route(self, route: Route | Layout) -> Path
```
...
Generated browser requests always use the reserved `/_pulse/*` namespace on the current origin. Socket.IO uses `/_pulse/socket.io`. Server-side loaders read `PULSE_SSR_BACKEND_URL` at runtime, so generated files contain no deployment address.
