...
- `max_cookie_bytes` - Maximum cookie size. Encoding raises if this limit is exceeded. Default: 3800.
...
Session values must be JSON-compatible. Store dates and datetimes as ISO
strings. Encoding invalid or oversized data raises without clearing the
session.
