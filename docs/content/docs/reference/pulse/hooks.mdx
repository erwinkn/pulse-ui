...
On first render, variables assigned inside the block are captured and stored. On subsequent renders, the block is skipped and variables are restored to their stored values.
