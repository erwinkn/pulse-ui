...
- `State` instances and other `Disposable` values captured in the block are disposed when the component unmounts, and when `key` changes
...
On first render, the code inside the block runs normally and variables are captured. On subsequent renders with the same key, the block is skipped entirely and variables are restored from storage. When the key changes, captured `Disposable` values (including `State`) are disposed, then the block runs again and replaces the captured variables.
