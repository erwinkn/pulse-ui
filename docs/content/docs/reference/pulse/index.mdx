...
| `global_state` | Access session-scoped shared state by type |
...
| `QueryKey` | Query key type (list/tuple of hashables) |
| `QueryKeys` | Wrapper for multiple query keys |
| `keys` | Helper to build `QueryKeys` |
...
| `FormScalar` | Non-container form value |
| `FormValue` | Native or structured form value |
...
| `Serializer` | Immutable serializer configuration |
| `SerializerAdapter` | Projection adapter for a Python type |
| `PulseSerializable` | Base class for self-projecting values |
| `WireMap` | Mapping that round-trips as a JavaScript `Map` |
...
| `require` | Register JS package version requirements |
...
Timer and task scheduling primitives. These are tracked and automatically cleaned up when the render session or app closes. See [Scheduling reference](/docs/reference/pulse/scheduling) for detailed documentation.

| Export | Description |
|--------|-------------|
| `later` | Schedule a callback after delay |
| `repeat` | Run a callback repeatedly at an interval |
