...
| `js_name` | `str` | Unique JS function name based on the callable name captured when wrapped (e.g., `"myFunc_1"`) |

Changing the Python callable's `__name__` after wrapping does not rename its emitted JavaScript symbol.
...
```python
from pulse import eval_js, javascript
...
async def on_check():
    value = await eval_js(get_value("#my-input"))
    print(value)
```
...
|-----------|------|---------|-------------|
| `expr` | `Expr` | required | Expression from calling a `@javascript` function |
| `timeout` | `float` | `10.0` | Maximum seconds to wait for the result |

### Errors

Raises `JsExecError` when client-side JavaScript throws an exception; see
[`JsExecError`](#jsexecerror). Raises `asyncio.TimeoutError` when no result
arrives before `timeout`.

### Reply protocol

A failed `eval_js(...)` arrives as `{type: "reply", error}` and rejects the future with `JsExecError`.

---

## JsExecError

Raised when client-side JavaScript execution fails.
