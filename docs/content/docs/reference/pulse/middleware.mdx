...
- `session` - Session data dictionary.
- `next` - Callable to continue the middleware chain. Returns the downstream decision; a downstream `Deny` is final.

**Returns:** `Ok[None]` to allow, `Deny` to block. A downstream `Deny` is final: the command never ran, so returning `Ok` cannot override it. Returning anything else raises `MiddlewareDecisionError` (fail closed).
