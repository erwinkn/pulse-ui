...
**Returns:** `Ok[None]` to allow, `Deny` to block. A downstream `Deny` is final: the command never ran, so returning `Ok` cannot override it. Returning anything else raises `TypeError` (fail closed).
