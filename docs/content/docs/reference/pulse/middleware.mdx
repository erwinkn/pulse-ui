...
Commands targeting the same route path (or the same channel) run in arrival order, even while this hook awaits. Commands for different paths, and replies, proceed concurrently.
