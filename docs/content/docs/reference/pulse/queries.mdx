...
On each State instance, `@query`, `@infinite_query`, and `@mutation` store a separate result wrapper per descriptor. This is independent of `QueryKey`: query keys still determine shared query data. Inheritance preserves the defining descriptor; a fresh same-name descriptor on a subclass gets a separate wrapper, including through `super()`. Assigning one descriptor object to multiple members or classes raises `TypeError`; create a fresh descriptor for each member.
...
| `retry_delay` | `float \| None` | `2.0` | Base delay (seconds) for exponential backoff between retries; doubles each attempt, capped at 30s |
