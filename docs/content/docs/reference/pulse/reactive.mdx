...
#### `discard(effect: Effect) -> None`

Remove an effect from this batch. Does nothing if it is not scheduled here.
...
### `resume(*, replace=False)`

Temporarily restore the tracking scope suspended by the active `Untrack` context:
...
print(scope.deps)  # Contains only `tracked`
```

`resume()` raises `RuntimeError` if the `Untrack` context is not active or did not suspend a tracking scope.

Pass `replace=True` to discard dependencies collected earlier in the suspended scope before tracking the block. This is useful when repeated attempts should keep only the final attempt's dependencies.
