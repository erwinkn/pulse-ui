...
If called outside render, a new handle is created. The handle attaches when you pass it to an element or ref callback. It stays attached until it disappears from the latest render or the route/session closes.
...
You can also pass a ref callback to an element. The callback receives the `Ref`
on mount and `None` on unmount.
...
The `ref` prop is serialized as an eval-only token: `#ref:{channelId},{refId}`.
