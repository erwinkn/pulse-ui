...
another thread requires the app to be running so Pulse can use the loop bound
during the FastAPI lifespan. A thread running a different event loop raises a
`RuntimeError`.
