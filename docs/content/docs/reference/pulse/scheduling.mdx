...
```python
from pulse.scheduling import later, repeat, create_task, call_soon
```
...
Route unmounting does not cancel `later()` callbacks. A callback scheduled from a
route continues to run unless you cancel its handle or the render session/app
closes. If that callback calls `ps.navigate()`, the navigation remains
route-bound by default and is ignored after the source route has unmounted.
...
`TimerHandleLike` for the scheduled callback.
...
`asyncio.Task[T]` that's tracked by the current scheduler.
...
- **Tracked** by the render session's scheduler or `App.scheduler`
- **Cancelled automatically** when the session closes or app shuts down
- **Cleaned up** from the scheduler when they complete
...
All scheduling primitives are scoped to the current **render session** (if inside a component render) or the **app** (if outside). The render session's scheduler and `App.scheduler` own the scheduled work. When the session closes or app shuts down:
...
These functions run inline on the loop that owns the scheduler. Scheduling from
another thread requires the owning loop to be running. A scheduler binds itself
to the first running loop that uses it, and `App.scheduler` is rebound to the
serving loop during FastAPI lifespan startup. A thread with no running loop can
schedule only after the scheduler has been used on its serving loop at least
once. A thread running a different event loop raises a
`RuntimeError`.
...
@app.fastapi.get("/refresh")
def refresh():
    background_work()
    return {"ok": True}
```

A handle returned from another thread can also be cancelled from that thread.
Cross-thread calls block until the loop picks up the work and raise
