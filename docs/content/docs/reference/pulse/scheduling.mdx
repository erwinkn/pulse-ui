...
`Task[None]` with `.cancel()` and await support.
...
class TypeaheadState(ps.State):
    query: str = ""
    _debounce_handle: ps.Task[None] | None = None
...
class LiveDataState(ps.State):
    data: dict = {}
    _poll_handle: ps.Task[None] | None = None
...
`Task[None]` that can be cancelled or awaited.
...
`Task[T]` owned by the current render session or app scheduler.
...
- **Owned** by the render session's or app's scheduler
- **Cancelled automatically** when the session closes or app shuts down
- **Drained** as part of scheduler shutdown
...
Each scheduler binds to the event loop where `await scheduler.start()` runs.
`App.scheduler` starts in the FastAPI lifespan, and a render session's
scheduler starts in `App.create_render`. The synchronous scheduling functions
work on that loop or from a thread without a running event loop. Calls from a
different running event loop, before `start()`, or after `close()` raise an
error.
