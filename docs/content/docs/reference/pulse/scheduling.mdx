...
These functions work on the owning event loop or from a thread without a
running event loop. The app scheduler binds to the serving loop at lifespan
startup; a render-session scheduler binds when it is first used on its loop.
Off-loop calls from a thread require that binding to have happened first.
Calls from a different running event loop raise an error.
