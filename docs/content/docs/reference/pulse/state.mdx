...
```python
class State(Resource, metaclass=StateMeta)
```
...
Clean up the state: calls `on_dispose()`, then disposes every resource the state owns. A state owns the resources created in its constructor and any unowned resource later assigned to one of its attributes (e.g. `self._poller = Effect(...)` from an event handler). Assigning over an owned resource disposes the replaced one.

`dispose()` cannot be overridden—a subclass that defines it raises `TypeError` at class definition. Override `on_dispose()` instead.
...
The diagnostic name is derived automatically: state methods use the class member name, standalone computeds use the callable name.
...
Each state instance caches one computed per descriptor. Inherited descriptors share the base definition; a fresh same-name descriptor on a subclass gets its own cache entry, so `super()` can access the base computed independently. Assigning one descriptor object to multiple members or classes raises `TypeError`; create a fresh descriptor for each member. Descriptors must be defined in the class body — assigning one to an existing State class also raises `TypeError`.
