...
# Define routes using the @app.route decorator (similar to FastAPI)
@ps.route("/")
def home():
...
def counter():
    ...
    return ps.div(
        ...
        ps.p("Note: This is a simple demo. State management would require additional implementation."),
        ps.p(
            ps.a("← Back to Home", href="/")
        )
    )
...
app = ps.App(routes=[*decorated_routes()])