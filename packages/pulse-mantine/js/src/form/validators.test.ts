...
describe("validators: splitValidationSchema", () => {
  it("collects server rules and builds client rules from specs", () => {
    ...
    const { clientRules, serverRulesByPath } = splitValidationSchema(
      schema as any
    );
    expect(serverRulesByPath["fieldA"]).toEqual([{ debounceMs: 150 }]);
    const nestedRule = (clientRules as any).nested.inner as Function;
    expect(
      nestedRule("ABC", { nested: { inner: "ABC" } }, "nested.inner")
    ).toBeNull();
  });
});
