...
import { applyUpdates, VDOMRenderer } from "./renderer";
import type { ComponentRegistry, RegistryEntry, VDOM } from "./vdom";
	...
	useEffect(() => {
		if (inBrowser) {
			client.mountView(path, {
				...
				onUpdate: (ops) => {
					setTree((prev) => (prev == null ? prev : applyUpdates(prev, ops, renderer)));
				},
			});
