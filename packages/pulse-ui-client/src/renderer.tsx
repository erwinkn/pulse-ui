...
export function applyUpdates(
	...
	for (const update of updates) {
		...
		const descend = (node: React.ReactNode, depth: number, path: string): React.ReactNode => {
			...
			switch (update.type) {
				...
				case "update_props": {
					...
					if (removedSomething) {
						...
						return createElement(element.type, nextProps, ...ensureChildrenArray(element));
					} else {
...
function rebindCallbacksInSubtree(
	...
	for (const key of Object.keys(baseProps)) {
		...
		if (renderer.hasRenderPropPath(propPath) && renderer.hasAnyCallbackUnder(propPath)) {
			nextProps[key] = rebindCallbacksInSubtree(baseProps[key], propPath, renderer);
		}
	}
