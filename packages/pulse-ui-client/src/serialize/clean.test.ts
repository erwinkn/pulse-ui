...
import { decodeFromWire, encodeForWire } from "./clean";
...
describe("encodeForWire / decodeFromWire", () => {
	...
	it("encodes FormData fields and File", async () => {
		...
		const files = Array.isArray(e.fields.file) ? e.fields.file : [e.fields.file];
		...
		const decoded = decodeFromWire(encoded) as any;
		const dfiles = Array.isArray(decoded.fields.file) ? decoded.fields.file : [decoded.fields.file];
