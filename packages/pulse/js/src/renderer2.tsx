...
export class VDOMRenderer2 {
	...
	#transformEvalProp(path: string, prop: string, value: VDOMPropValue) {
		...
		if (isExprNode2(value)) return this.#evalExpr(value, {});
	...
	renderNode(node: VDOMNode, currentPath = ""): ReactNode {
		...
		// expr
		if (isExprNode2(node)) return this.#evalExpr(node, {}) as ReactNode;
		...
		if (process.env.NODE_ENV !== "production") {
			console.error("Unknown VDOM v2 node:", node);
		}
		return null;
	}
	...
	applyUpdates(initialTree: ReactNode, updates: VDOMUpdate2[]): ReactNode {
		...
		for (const update of updates) {
			...
			const descend = (node: ReactNode, depth: number, path: string): ReactNode => {
				...
				switch (update.type) {
					...
					default:
						throw new Error(`[Pulse renderer2] Unknown update type: ${(update as any)?.type}`);
				}
			};
