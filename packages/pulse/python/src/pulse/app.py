...
class App:
	...
	def setup(self, server_address: str):
		...
		def wrap_html_with_shell(html_content: str, state: Any) -> str:
			"""
			Wrap Bun's rendered HTML in a full document shell with state injection.

			Args:
			    html_content: HTML generated by Bun's renderVdom
			    state: Serialized state to inject into __PULSE_DATA__ script

			Returns:
			    Complete HTML document with injected state
			"""
			# Serialize the state for client hydration
...
<head>
	...
	<script id="__PULSE_DATA__" type="application/json">{state_json}</script>
</head>
<body>
				...
				try:
					...
					async with httpx.AsyncClient() as client:
						...
						try:
							...
							# Wrap HTML in full document shell with state injection
							state_to_inject = {
								"vdom": vdom,
								"routeInfo": route_info,
							}
