"""Core infrastructure for JavaScript module bindings in transpiler_v2.
...
class JsModule(ExprNode):
	...
	Attributes:
		...
		py_name: Python module name for error messages (e.g., "pulse.js2.math")
	...
	def to_expr(self) -> Identifier | Import:
		...
		return Import(self.name, self.src, is_default=self.kind == "default")
	...
	def register(  # pyright: ignore[reportIncompatibleMethodOverride]
		*,
		...
		"""Register the calling Python module as a JavaScript module binding.
			...
			dependency (e.g., `import pulse.js2.math as Math`) during transpilation.
