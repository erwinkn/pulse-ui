...
class TestArrowEmit:
	...
	def test_single_statement(self):
		from pulse.transpiler.nodes import Return

		arrow = Arrow([], [Return(Literal(42))])
		assert emit(arrow) == "() => { return 42; }"

	def test_multiple_statements(self):
		from pulse.transpiler.nodes import Assign, Return
	...
	def test_throw_statement(self):
		from pulse.transpiler.nodes import New, Throw
...
class TestCallablePropError:
	...
	def test_callable_prop_raises(self):
		...
		elem = Element("button", {"onClick": handler})
...
class TestExprOf:
	...
	def test_set(self):
		from pulse.transpiler.nodes import New
...
class TestExprRegister:
	...
	def test_register_callable(self):
		...
		from pulse.transpiler.nodes import Transformer
