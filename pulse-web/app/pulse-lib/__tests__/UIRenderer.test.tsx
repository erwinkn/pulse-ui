...
describe("UIRenderer Component Logic", () => {
  ...
  it("should create mount point nodes with children", () => {
    const mountPointNode = createMountPoint(
      "card",
      {
        title: "Test Card",
      },
      ["Text child", createElementNode("span", {}, ["Element child"])]
    );
  ...
  it("should handle nested structures correctly", () => {
    const nestedNode = createElementNode("div", { "data-testid": "outer" }, [
      createElementNode("span", { "data-testid": "inner" }, ["Nested content"]),
    ]);
  ...
  it("should handle nested fragments correctly", () => {
    const nestedFragments = createElementNode(
      "div",
      { "data-testid": "wrapper" },
      [
        createFragment([
  ...
  it("should validate reserved fragment tag usage", () => {
    expect(() => {
      ...
      `Tags starting with '$$' are reserved for internal use. Please use a different tag name.`
    );
  });
  ...
  it("should handle mixed content with mount points", () => {
    ...
    expect(getMountPointComponentKey(mixedContent.children[3] as any)).toBe(
      "counter"
    );

    expect(isMountPointNode(mixedContent.children[4])).toBe(true);
    expect(getMountPointComponentKey(mixedContent.children[4] as any)).toBe(
      "user-card"
    );
  });
