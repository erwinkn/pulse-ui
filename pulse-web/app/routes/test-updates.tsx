...
import { ReactiveUIContainer } from "~/pulse-lib/pulse";
...
import { UIRenderer } from "~/pulse-lib/renderer";
import type { ComponentType } from "react";
import type { VDOMUpdate, VDOMNode } from "~/pulse-lib/vdom";
...
interface TestableUIContainerProps {
  ...
  onUpdateMethodsReady?: (methods: {
    applyBatchUpdates: (updates: VDOMUpdate[]) => void;
  }) => void;
}
...
const TestableUIContainer = forwardRef<any, TestableUIContainerProps>(
  ({ initialTree, callbackInfo, transport, onUpdateMethodsReady }, ref) => {
    ...
    const processedTree = React.useMemo(() => {
      ...
      function processNode(node: VDOMNode): VDOMNode {
          ...
          children: [
            {
              ...
              props: {},
              children: [
                "Insert Item: Adds a new item to the dynamic content area",
              ],
            },
            {
              tag: "li",
            ...
            },
            {
              ...
              props: {},
              children: [
                "Replace First Item: Replaces the first item with new content",
              ],
            },
            {
              tag: "li",
              props: {},
              children: [
                "Update Container Props: Changes the container's border color",
              ],
            },
          ],
        },
...
export default function TestUpdatesRoute() {
  ...
  const applyReplaceUpdate = () => {
    const replaceUpdate: VDOMUpdate = {
      type: "replace",
  ...
  const applyUpdatePropsUpdate = () => {
    const updatePropsUpdate: VDOMUpdate = {
      type: "update_props",
      path: [1], // Target the test-container div
      data: {
        props: {
