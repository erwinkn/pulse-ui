...
# Type definitions
VDOMNode = Union[Node, str, int, bool, float]
UpdateOperation = Dict[str, Any]
Path = List[int]


def diff_vdom(old_node: Optional[VDOMNode], new_node: Optional[VDOMNode], path: Path = None) -> List[UpdateOperation]:
    """
    ...
    # Diff children
    children_operations = diff_children(old_element.children, new_element.children, path)
    operations.extend(children_operations)
    
    return operations


def diff_props(old_props: Dict[str, Any], new_props: Dict[str, Any], path: Path) -> List[UpdateOperation]:
    """
...
def diff_children(old_children: List[VDOMNode], new_children: List[VDOMNode], path: Path) -> List[UpdateOperation]:
    """
    ...
    has_keyed_old = any(isinstance(child, Node) and child.key is not None for child in old_children)
    has_keyed_new = any(isinstance(child, Node) and child.key is not None for child in new_children)
...
def _diff_keyed_children(old_children: List[VDOMNode], new_children: List[VDOMNode], path: Path) -> List[UpdateOperation]:
    ...
    for new_index, new_child in enumerate(new_children):
        ...
        if isinstance(new_child, Node) and new_child.key is not None:
            ...
            if (new_index < len(old_children) and 
                new_index not in used_old_positions and
                not (isinstance(old_children[new_index], Node) and old_children[new_index].key is not None)):
    ...
    for old_index, old_child in enumerate(old_children):
        if (old_index not in used_old_positions and 
            not (isinstance(old_child, Node) and old_child.key is not None)):
            operations.append({
                "type": "remove",
                "path": path + [old_index]
            })
...
def _diff_positional_children(old_children: List[VDOMNode], new_children: List[VDOMNode], path: Path) -> List[UpdateOperation]:
...
def _serialize_node(node: VDOMNode) -> Union[Dict[str, Any], str, int, bool, float]:
    """
...
def optimize_operations(operations: List[UpdateOperation]) -> List[UpdateOperation]:
    """
