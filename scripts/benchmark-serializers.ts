...
function createLargeDefaultPayload(targetSerializedBytes = 500_000) {
	...
	const root: Record<string, any> = {
		...
		metadata: {
			version: "v3",
			featureFlags: new Set(["deep", "large", "circular", "dates", "sets", "maps"]),
			stats: new Map([
		...
		uniqueVisitors: new Set(
			Array.from({ length: 500 }, (_, index) => `visitor-${index.toString().padStart(4, "0")}`),
		),
		preferences: new Map([
			["theme", "dark"],
			["layout", "wide"],
	...
	root.recordsById = new Map(root.records.map((record: any) => [record.id, record]));
	root.recordSet = new Set(root.records.slice(0, Math.min(20, root.records.length)));
...
function benchmarkWithSize<T>(label: string, fn: () => { result: T; serialized: unknown }) {
...
function main() {
	...
	const rows = [v2Roundtrip, v3Roundtrip].map(({ label, avg, min, max, runs }) => ({
		label,
		runs,
