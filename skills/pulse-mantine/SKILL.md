...
**Key rules:**
- Children as first positional args: `Button("text")`, `Title("heading")`
...
| Validator | Usage |
|-----------|-------|
| `IsNotEmpty(err)` | Required field |
| `IsEmail(err)` | Email format |
| `HasLength(min=, max=, exact=)` | String length |
| `Matches(pattern, err)` | Regex pattern |
| `IsInRange(min=, max=)` | Number range |
| `IsDate()` / `IsISODate()` | Date validation |
| `IsBefore(field=)` / `IsAfter(field=)` | Date comparison |
| `MatchesField(field, err)` | Field equality |
| `ServerValidation(fn)` | Async server check |
| `RequiredWhen(field, equals=)` | Conditional required |
